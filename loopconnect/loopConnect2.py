...
COMMANDs = ['date', 'fsclish -c "add networking address /MMN-0 iface intmsg0 ip-address 1.1.1.1/28"', 'fsclish -c "show networking address iface intmsg0 ip-address 1.1.1.1"', 'fsclish -c "delete networking address /MMN-0 iface intmsg0 ip-address 1.1.1.1"', 'fsclish -c "show networking address iface intmsg0"']
START_HOUR = 8
STOP_HOUR = 19
PERIOD = 5   # s 
    ...
    try:
        ...
        while (times <100):
            for command in COMMANDs:
                print "haha1"
                excuteSCLICommand(ssh,command,times)
            sleep(PERIOD)
            times = times +1
    except pexpect.EOF:
        print "EOF"
